...
以下のサンプルソースを **Form.js** に記載（すべて上書き）し、index.jsで読み込んで `npm run start` で実行してみましょう  
...
以下のサンプルソースを **Counter.js** に記載し、index.jsで読み込んで `npm run start` で実行してみましょう  
...
``` js
// index.js でCounterを読み込むように修正
import React, { StrictMode } from "react";
import { createRoot } from "react-dom/client";
import "./css/styles.css";

import Form from "./Form";
import Counter from "./Counter";  // 追加
...
以下のサンプルソースを **Counter.js** に記載し、 `npm run start` で実行してみましょう  
